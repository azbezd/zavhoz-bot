...
def looks_like_inventory_update(text: str, has_photo: bool) -> bool:
    ...
    if "?" in lowered or lowered.startswith(question_starts):
        return False
    # «провода остались», «дюпоны есть» — вопрос о наличии без «?», не изменение склада.
    if lowered.rstrip("?!. )").endswith(("есть", "имеется", "в наличии", "остался", "осталось", "остались")):
        return False
    markers = [
        "купил",
        "купила",
...
def _looks_like_stock_question(text: str) -> bool:
    ...
    if any(c in low for c in cues):
        return True
    # Просьба дать ссылку/карточку на позицию — отвечаем нашей карточкой (детерминированно),
    # а не свободной моделью, которая может выдать ссылку на магазин или вовсе без ссылки.
    if "ссылк" in low or "карточк" in low:
        return True
    # Вопрос со знаком «?» и словом наличия в любом месте: «так а УЫЗ у нас какие?»,
    # «есть чем мерить напряжение?». Если классификатор ничего не выберет — уйдёт в чат.
    if "?" in low and re.search(r"\b(какие|какой|какая|сколько|есть)\b", low):
        return True
    stripped = low.rstrip("?!. )")
